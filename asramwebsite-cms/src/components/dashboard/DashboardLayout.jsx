...
const DashboardLayout = ({ children, showSidebar = true }) => {
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-white font-sans">
            ...
            <div className={`fixed top-0 left-0 right-0 h-[60px] bg-white z-40 flex items-center justify-between px-4 shadow-sm md:hidden ${!showSidebar ? 'hidden' : ''}`}>
            ...
            <div className={`flex-1 h-screen overflow-y-auto bg-white ${showSidebar ? 'pt-[60px] md:pt-0' : ''}`}>
                {children}
            </div>
        </div>
    );
