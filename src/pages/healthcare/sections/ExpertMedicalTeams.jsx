...
const ExperiencedDoctors = () => {
    return (
        <PageSection bg="bg-white" paddingClass="py-[80px] px-[0px]">
            {/* Main Flex Wrapper: Items Center as per visual alignment of text block center to image center area, or just stretch. Figma snippet says 'align-items: center' */}
            <div className="flex flex-col lg:flex-row justify-between items-center gap-[60px] lg:gap-[160px]">

                {/* Left Column: Heading and List */}
                {/* Width 460px from Figma Frame-258 width */}
                <div className="w-full lg:w-[460px] flex-shrink-0 flex flex-col items-start">
                    <h3 className={`font-montserrat font-bold text-[32px] text-[#223F7F] mb-[40px] leading-tight`}>
                        Experienced Doctors
                    </h3>

                    {/* List Items Container: Gap 12px from Figma */}
                    <div className="flex flex-col gap-[12px] w-full">
                        {features.map((item, index) => (
                            // List Item Row: Gap 12px
                            <div key={index} className="flex items-start gap-[12px] w-full">
                                {/* Icon Box: 45.1px width/height, 7.5px radius, bg-[#223F7F]/10 */}
                                ...
                                <div className="flex flex-col gap-[2px]">
                                    {/* Title: 18px, Medium (500), #223F7F */}
                                    <h4 className={`font-montserrat font-medium text-[18px] text-[#223F7F]`}>
                                        {item.title}
                                    </h4>
                                    {/* Desc: 14px, Normal (400), #191919 with 75% opacity (approx #191919BF), leading 1.43 (~20px) */}
                                    <p className={`font-montserrat font-normal text-[14px] text-[#191919BF] leading-[20px] max-w-[402.9px]`}>
                                        {item.desc}
                                    </p>
                                </div>
                            </div>
                ...
                {/* Right Column: Image */}
                {/* Width 580px, Height 386.7px, Radius 10.7px */}
                <div className="w-full lg:w-[580px] h-[300px] lg:h-[386.7px] flex-shrink-0">
                    <img
                        src={TeamImg}
