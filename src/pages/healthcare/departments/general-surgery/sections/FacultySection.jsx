...
import { T } from "@/theme";
import DrRajeshImg from "@/assets/departments/departfac.png"
...
const FacultySection = () => {
    ...
    return (
        <PageSection bg="bg-[#EEF2F7]" paddingClass="py-[80px]">
            ...
            {/* Faculty List */}
            <div className="flex flex-col gap-[40px] mb-[40px] w-full">
                {faculty.map((member, index) => (
                    <div key={index} className="bg-white rounded-[12px] p-[30px] flex flex-col md:flex-row gap-[30px] border border-[#E0E0E0] shadow-sm">

                        {/* Image */}
                        <div className="w-[210.27px] h-[240.31px] rounded-[11.26px] overflow-hidden flex-shrink-0 ">
                            <img
                                src={member.image}
                                alt={member.name}
                        ...
                        <div className="flex-1 flex flex-col items-start h-full self-stretch">
                            <h3 className="text-[#223F7F] font-semibold text-[28px] mb-[8px] font-montserrat">{member.name}</h3>
                            <p className="text-[#191919] text-[18px] font-normal leading-[1.44] font-montserrat mb-[8px]">{member.role}</p>
                            <p className="text-[#191919C0] text-[14px] font-normal font-montserrat mb-[12px]">{member.qualification}</p>

                            <div className="grid grid-cols-2 gap-[20px] mb-[12px] w-full max-w-[500px]">
                                {/* Experience */}
