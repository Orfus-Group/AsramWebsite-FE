...
export default function StudentExperience({
  ...
  return (
    <PageSection bg="bg-[#FFF]" paddingClass="py-[60px] md:py-[80px]">
      ...
      <div className="flex flex-col lg:flex-row gap-[30px] lg:gap-[40px] items-start mb-[60px] w-full mx-auto">
        ...
        <div className="w-full lg:flex-1">
          <img
            src={imageSrc}
            alt={heading}
            className="w-full h-auto lg:h-[361px] block object-cover rounded-[8px]"
          />
        </div>
          ...
          <div className="flex flex-col gap-[20px] w-full">
            {Array.isArray(description) ? (
              description.map((para, idx) => (
                <p key={idx} className="font-montserrat font-normal text-[16px] md:text-[17px] lg:text-[18px] leading-[24px] md:leading-[26px] lg:leading-[26px] text-justify text-[#191919BF] m-0">
                  {para}
                </p>
              ))
            ) : (
              <p className="font-montserrat font-normal text-[16px] md:text-[17px] lg:text-[18px] leading-[24px] md:leading-[26px] lg:leading-[26px] text-justify text-[#191919BF] m-0">
                {description}
              </p>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-[20px] xl:gap-x-[20px] xl:gap-y-[20px] w-full justify-items-center xl:justify-items-stretch">
        {features.map((item, i) => (
          <InfoCard
            key={i}
            icon={item.icon}
